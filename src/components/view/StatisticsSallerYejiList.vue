<template>
  ...
  <div class="block" style="margin-top:10px;">
      <el-select v-model="value2" clearable placeholder="分配经纪人" @change="allotBatch" size="small">
       <el-option
        v-for="item in options2"
        :key="item.id"
        :label="item.name"
...
export default {
  ...
  methods: {
    ...
    exportExcel () {
      ...
      let data = []
      APIOrder.statisticsSallerYejiList({page: 1, pageSize: 10000}).then(response => {
        if (response.code === 0) {
          let result = response.data.pageBean.result
          if (result.length > 0) {
            result.forEach(ele => {
