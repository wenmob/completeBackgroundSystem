  ...
  methods: {
    ...
    checkHasId () {
      ...
      if (this.$route.query.id) {
        ...
        APIMerchant.merchantSetProductContractGetById(id).then(response => {
          if (response.code === 0) {
    ...
    submitForm (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          APIMerchant.merchantSetProductContractSave(that.formLabelAlign).then(response => {
            if (response.code === 0) {
              this.$message({
                message: '提交成功',
                type: 'success'
              })
              this.$router.push('/admin/merchantSetProductContract/list')
            } else {
              this.$message({
